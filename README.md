...
| **Latency** | UserPromptSubmit hook fast path: **3–4 ms** cold (median of 5 runs, Windows 11 / Node 22, v2.4.0). Reproduce (from repo root): `echo '{"hook_event_name":"UserPromptSubmit","prompt":"hello"}' \| TOKE_ROOT=$PWD/toke node plugins/godspeed/hooks/brain_hook_fast.js` and time it. Note: exit 2 with no output is the by-design fall-back-to-Python signal when `TOKE_ROOT` is unset — not a crash. Re-measure on your system — hook timing is environment-sensitive. |
| **Tests** | **35/35 pass** on the v2.4.3 Homer infrastructure (measured 2026-07-05): `test_cost_guard.py` 13/13 + `test_token_accountant.py` 22/22. Reproduce: `python plugins/godspeed/automations/homer/test_cost_guard.py && python plugins/godspeed/automations/homer/token_accountant/test_token_accountant.py`. |
| **Footprint** | Plugin install (Option A): **1.6 MB** on disk, 18 skills, 5 lifecycle hooks, ~13K LOC of Python under `automations/`. install.sh path (Option B): 2.7 MB, 16 skills (different curated set — see [What each install path includes](#what-each-install-path-includes)). |
