...
- **Detail**: Never propagate metadata from a secondary source without opening the primary. A real failure mode: a routing index claimed a PDF section was at pages "~181-220" when the PDF was actually only 36 pages total — the bogus page count came from misreading an aggregate "total docs pages" stat in a secondary index. The error was caught only by a verification test that errored on `pdftoppm first page (181) can not be after last page (36)`. Opening a PDF to verify costs ~3 seconds + 1K tokens. The cost of propagating a wrong number is the whole system's correctness score.
...
- **Confirmed by**: project-init Session #2 (CUDA install), Session #4 (bitsandbytes NF4 bf16 leak), another-ml-project (implicit — same hardware)
...
<!-- meta: { "run_id": "cycle_vault_audit_20260409", "domain": "skills", "confidence": "HIGH", "confirmed_count": 1, "roi_score": 5, "staleness_check": "2026-04-09" } -->
